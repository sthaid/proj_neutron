...
//
// FYI, the conversion from ADC value to voltage is:
//       8  =  -10000 mV
//    2048  =       0 mV
//    4095  =   10000 mv
...
int32_t mccdaq_callback(uint16_t * d, int32_t max_d)
{
    ...
    // once per TIME_INTVL_SECS seconds do the following ...
    uint64_t time_now = time(NULL);
    static uint64_t time_last_published;
    if (time_now >= time_last_published+TIME_INTVL_SECS) {    
        int32_t mccdaq_restart_count;

        // publish the total_pulses for this interval
        publish(time_now, total_pulses);
        ...
        if (mccdaq_restart_count > (2*TIME_INTVL_SECS) ||
            max_data/TIME_INTVL_SECS < 460000 || max_data/TIME_INTVL_SECS > 520000 ||
